...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- **REUSE BEFORE CREATING + NO DUPLICATE NAMES (Rule 63, P0).** Before defining ANY new "
        "component, snippet, or function, search the THREE tiers and confirm the name is not already "
        "taken — defining `export const <Name>` whose name already exists somewhere in scope silently "
        "shadows the original (DOMQL bare keys and `el.call('fn', …)` both resolve through merged "
        "context: built-ins → shared libraries → project) and breaks every other consumer that expected "
        "the canonical version. Search order: **(1) framework built-ins** (`@symbo.ls/default-config` — "
        ...
        "auto-resolves through the merged context so the bare key works whichever tier defined it. **If "
        "your intended name collides with an entry in any tier and the shape is genuinely different, "
        "RENAME your new component/function — do not shadow.** The only time a same-name redefinition is "
        "acceptable is when you explicitly intend to override a shared-library key project-wide (local "
        "always wins on collision); make that an explicit, documented decision, never an accidental "
        "shadow. If you're about to define a third near-duplicate (e.g. `UserCard` / `MemberCard` / "
        "`ProfileCard` with the same shape), STOP and refactor: lift the shared shape into ONE canonical "
        "component at `components/<Name>.js` and have the page-level wrappers reference it by bare key. "
        "Same for functions. **Never edit shared-library files** to fix a mismatch; override at the "
        "consumer level. Use `mcp__symbols-mcp__search_symbols_docs(query)` for semantic search across "
        "the catalog, OR `cat sharedLibraries.js` + `grep -rEn '^export const [A-Z]' <resolved-lib-paths>/ "
        "components/ snippets/` (and `'^export (const|function) [a-z]'` for functions/methods). Treat "
        "duplication and silent shadows as a P0 cleanup, not a 'TODO later'.\n"
        ...
        "- **NO GLOBALS — never assign to `window` / `globalThis` / `document` (Rule 64, P0).** No "
        "`window.X = …`, no `globalThis.X = …`, no `document.title = …`, no `document.body.classList.add(…)`, "
        "no `window.__projectInit = …`. Globals are not reactive, leak across iframes (canvas/workspace/preview "
        "share one `window` in dev), hold GC references forever, and are stripped by frank serialization "
        "(FA513/FA514) so they silently no-op in prod. Use the four canonical channels: cross-component data "
        "→ root state (`el.getRootState().update({ … })` + `s.root.X` reads); per-instance non-reactive "
        "storage → `el.scope.X = …` in `onInit`, clean up in `onRemove`; project functions → `el.call('fn', …)` "
        "(never `window.fn`); reach other parts of the tree → `el.lookup('Key')` / `el.lookdown('Key')` / "
        "`el.getRoot()`. Page title/meta → `metadata: {…}` (helmet); theme swap → `changeGlobalTheme()`; CSS "
        "vars → `vars: { '--x': v }`; classes → `class: { open: (el, s) => s.isOpen }`; navigation → "
        "`el.router(path, el.getRoot())`. Tolerated read-only exceptions: `window.location` reads (never writes) "
        "and, for genuine browser events (`resize`, `beforeunload`, `hashchange`, `storage`) or events that "
        "never reach an element you own (foreign portals, outside-click), the flat `onWindowXxx:` / "
        "`onDocumentXxx:` props on the owning component — DOMQL registers once and tears down on dispose; "
        "raw `window.addEventListener` / `document.addEventListener` is FA503 even there. Anything "
        "else — including devtools hooks — must be added at the framework level via a ticket in "
        "`FRAMEWORK_TICKETS.md`, never as a project-side `window.__X = …` shim.\n"
